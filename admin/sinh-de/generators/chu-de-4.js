...
function q1(type,rng){const a=ri(2,12,rng),b=ri(2,12,rng),k=ri(2,5,rng);const ans=`${a}/${b}`;return make(1,type,`Rút gọn tỉ số ${a*k}/${b*k}.`,ans,`Chia cả tử và mẫu cho ${k}.`,[`${b}/${a}`,`${a*k}/${b}`,`${a}/${b*k}`],2,rng)}
...
function q5(type,rng){const ab=ri(3,10,rng),ac=ri(3,10,rng),dc=ri(2,8,rng),db=dc*ab/ac;if(!Number.isInteger(db))return q5(type,rng);return make(5,type,`Trong tam giác ABC, AD là phân giác góc A. Biết AB=${ab} cm, AC=${ac} cm, DC=${dc} cm. Tính DB.`,`${db} cm`,'Theo tính chất phân giác: DB/DC = AB/AC.',[`${db+1} cm`,`${Math.max(1,db-1)} cm`,`${dc} cm`],4,rng)}
