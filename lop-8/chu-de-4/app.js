...
function newQuestion(){ currentQuestion=generateQuestion(currentLevel); questionSubmitted=false; instruction.textContent=currentQuestion.prompt; expression.textContent=pretty(currentQuestion.expression); feedback.className="feedback hidden"; feedback.innerHTML=""; answerArea.innerHTML="";
...
async function submitAnswer(){ let userAnswer=""; if(currentQuestion.type==="text"){userAnswer=document.getElementById("answerInput").value;if(!userAnswer.trim())return showFeedback(false,"Bạn chưa nhập đáp án.",false);}else{const selected=document.querySelector('input[name="choice"]:checked');if(!selected)return showFeedback(false,"Bạn chưa chọn đáp án.",false);userAnswer=selected.value;}
 const isCorrect=equivalent(userAnswer,currentQuestion); if(!questionSubmitted){questionSubmitted=true;totalCount++;if(isCorrect)correctCount++;updateScore();await recordCloudProgress(isCorrect);} const message=isCorrect?"Chính xác. Em đã chọn đúng tỉ lệ hoặc định lí cần dùng.":`${diagnose(userAnswer,currentQuestion)} Đáp án đúng là ${pretty(currentQuestion.answer)}.`; showFeedback(isCorrect,message,true); }
function showFeedback(ok,message,withExplanation){feedback.className=`feedback ${ok?"correct":"incorrect"}`;feedback.innerHTML=`<strong>${escapeHtml(message)}</strong>${withExplanation?`<span>${escapeHtml(currentQuestion.explanation)}</span>`:""}`;}
